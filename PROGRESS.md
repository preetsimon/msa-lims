...
**Updated:** 2026-08-22 · **Phase:** 0 complete (walking skeleton)
...
| 1 · The spine | wk 2–4 | Next — submission → result → certificate, end to end |
...
**Health:** 127 tests passing · ruff clean · mypy `--strict` clean · migrations
...
- Auth is **not implemented yet** — the settings and role model are in place and
  the OIDC pattern is inherited from Sentinel, but no dependency enforces it.
  This is the largest outstanding item and must land in Phase 1, alongside the
...
### Tests — 127
- **Unit** (86): units and dimensions, censored values, assay arithmetic,
  sample labels and intervals, the state machine.
...
1. **Authentication before the first write endpoint.** Port Sentinel's OIDC
   module and the dev-header shim, including the test that a forged
   `X-Actor-Role` is ignored once `auth_mode=oidc`.
